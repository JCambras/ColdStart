...
export function NearbyPicker({ label, icon, places, selected, onSelect, onClear, costValue, onCostChange }: NearbyPickerProps) {
  ...
  return (
    <div>
      <label style={{ fontSize: 12, fontWeight: 600, color: colors.textSecondary, display: 'block', marginBottom: spacing[4] }}>{icon} {label}</label>
      {selected ? (
        <div style={{ border: `1px solid ${colors.brandLight}`, background: colors.bgInfo, borderRadius: 10, padding: pad(spacing[10], spacing[14]) }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: colors.brandDeep }}>{selected.name}</div>
              <div style={{ display: 'flex', gap: spacing[8], alignItems: 'center', marginTop: spacing[2] }}>
          ...
          <input value={costValue} onChange={(e) => onCostChange(e.target.value)}
            ...
            style={{ width: '100%', padding: pad(spacing[6], spacing[10]), fontSize: 12, border: `1px solid ${colors.borderMedium}`, borderRadius: 8, outline: 'none', boxSizing: 'border-box' as const, marginTop: spacing[8] }} />
        </div>
      ) : places.length > 0 ? (
        <div>
          <button onClick={() => setOpen(!open)} aria-expanded={open} style={{
            width: '100%', padding: pad(spacing[10], spacing[14]), fontSize: 13, color: colors.textTertiary,
          ...
          {open && (
            <div role="listbox" aria-label={`Nearby ${label.toLowerCase()} options`} style={{ border: `1px solid ${colors.borderDefault}`, borderRadius: 10, overflow: 'hidden', maxHeight: 200, overflowY: 'auto', marginTop: spacing[4] }}>
              {places.map((p, i) => (
                <div key={i} role="option" aria-selected={false} onClick={() => { onSelect(p); setOpen(false); }}
                  ...
                  style={{ padding: pad(spacing[8], spacing[14]), cursor: 'pointer', borderBottom: `1px solid ${colors.borderLight}`, background: colors.surface, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: colors.textMuted, flexShrink: 0, marginLeft: spacing[8] }}>{p.distance}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ padding: pad(spacing[10], spacing[14]), fontSize: 13, color: colors.textMuted, border: `1px dashed ${colors.borderDefault}`, borderRadius: 10, textAlign: 'center' }}>
