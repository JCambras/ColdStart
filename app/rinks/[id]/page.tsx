      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 24px' }}>
        ...
        <section style={{ paddingTop: 24, paddingBottom: 0 }}>
          ...
          {hasData && (() => {
            ...
            const totalCount = allSignals.reduce((sum, s) => sum + s.count, 0);
            const avgCount = allSignals.length > 0 ? Math.round(totalCount / allSignals.length) : 0;
            const confidence = avgCount >= 30 ? 'High' : avgCount >= 10 ? 'Medium' : 'Low';
            const confidenceColor = avgCount >= 30 ? '#059669' : avgCount >= 10 ? '#d97706' : '#9ca3af';
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {topSignals.map((s) => {
                    ...
                    const pct = noData ? 0 : Math.round(((s.value - 1) / 4) * 100);
                    const barColor = noData ? '#d1d5db' : getBarColor(s.value);
                    return (
                      <div key={s.signal} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <span style={{ width: 90, flexShrink: 0, fontSize: 13, fontWeight: 600, color: '#374151' }}>
                          {meta?.label || s.signal}
                        </span>
                        <div style={{ flex: 1, height: 6, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{ width: `${pct}%`, height: '100%', borderRadius: 3, background: barColor, transition: 'width 0.6s ease' }} />
                        </div>
                        <span style={{ width: 28, textAlign: 'right', fontWeight: 700, fontSize: 13, color: noData ? '#9ca3af' : '#374151' }}>
                          {noData ? '—' : s.value.toFixed(1)}
                        </span>
                      </div>
                    );
                  })}
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 11, color: '#9ca3af' }}>
                    {avgCount} avg. rating{avgCount !== 1 ? 's' : ''} per signal
                  </span>
                  <span style={{
                    fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10,
                    color: confidenceColor,
                    background: avgCount >= 30 ? '#ecfdf5' : avgCount >= 10 ? '#fffbeb' : '#f9fafb',
                    border: `1px solid ${avgCount >= 30 ? '#a7f3d0' : avgCount >= 10 ? '#fde68a' : '#e5e7eb'}`,
                  }}>
                ...
                {summary.tips.length > 0 && (
                  <p style={{
                    fontSize: 12, color: '#6b7280', lineHeight: 1.45, margin: '8px 0 0',
                    fontStyle: 'italic',
                  }}>
